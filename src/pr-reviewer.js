...
export async function reviewPullRequest(owner, repo, prNumber) {
  ...
  for (const file of pr.files) {
    ...
    try {
      ...
      console.error(
        `[ERROR:ANALYSIS] Failed to analyze ${file.filename}:`,
        error.message,
 ...
 * - Emojis should precede text labels (e.g., '✅ **Status**') to ensure
 *   screen readers announce the visual indicator before the label.
 * - This provides context before the main content for assistive technology users.
 * 
 * @param {Array} results - Array of review results per file
 * @param {Array} allIssues - Flattened array of all issues found
 * @returns {string} Markdown-formatted comment body optimized for screen readers
 */
function generateReviewComment(results, allIssues) {
  ...
  for (const result of results) {
    ...
    for (const issue of result.issues) {
      ...
      if (issue.fixedCode) {
        ...
        // Default to 'text' for unknown file types to avoid implying incorrect structure
  ...
  comment += `\n> 🤖 **Auto-generated review:** This review was automatically generated by [AccessLint PR Bot](https://github.com/tryzens/accesslint-pr-bot). `;
 ...
 * 
 * Creates a new branch with automated fixes for accessibility issues found in the original PR.
 * The fix PR targets the same branch as the original PR for easy integration.
 * 
 * Emoji Accessibility Pattern:
 * - Emojis precede text labels to ensure screen readers announce visual indicators
 *   before the main content, providing better context for assistive technology users.
 * 
 * @param {string} owner - Repository owner
 * @param {string} repo - Repository name
 * @param {number} originalPRNumber - Original PR number being reviewed
 * @param {Object} originalPR - Original PR object
 * @param {Array} filesToFix - Array of files with fixes to apply
 * @returns {Promise<Object>} Created fix PR object
 */
async function createFixPullRequest(owner, repo, originalPRNumber, originalPR, filesToFix) {
  ...
  try {
    ...
    const fixPR = await createPullRequest(
      ...
      `\n\n---\n` +
      `🤖 **Auto-generated by AccessLint PR Bot:** [AccessLint PR Bot](https://github.com/tryzens/accesslint-pr-bot)\n\n` +
    ...
    console.error(
      '[ERROR:PR_CREATION] Failed to create fix PR:',
      error.message,
      process.env.NODE_ENV === 'development' ? error.stack : ''
    );
    throw error;
 ...
 * @returns {boolean} True if file should be reviewed for accessibility
 */
 ...
 * 
 * @param {string} filename - File name to extract language from
